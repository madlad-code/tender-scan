...
def test_read_huddinge_ledger_keeps_the_account_and_the_cost_centre():
    ...
    assert row.supplier_orgnr == "812800-3624"
    assert (row.period_year, row.period_month) == (2026, 5)
...
def test_read_huddinge_ledger_needs_its_columns():
    blob = _xlsx({"Blad1": [["Period", "Belopp"], [202601, 5]]})
    with pytest.raises(municipal.ReaderError):
        municipal.read_huddinge_ledger(blob)


def test_huddinge_is_registered_as_a_ledger_source():
    assert municipal.LEDGERS["huddinge"].buyer_org == "Huddinge kommun"
